...
-- Galaxy map (one row per user — main planet coords for the galaxy view)
...
-- Fleet missions (spy / attack / colonize)
CREATE TABLE IF NOT EXISTS fleet_missions (
  id              TEXT PRIMARY KEY DEFAULT (lower(hex(randomblob(16)))),
  user_id         TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  target_user_id  TEXT,
  mission_type    TEXT NOT NULL,   -- 'spy' | 'attack' | 'colonize'
  target_coords   TEXT NOT NULL,
  target_name     TEXT NOT NULL DEFAULT 'Ismeretlen',
  status          TEXT NOT NULL DEFAULT 'travelling',  -- 'travelling' | 'done'
  result          TEXT NOT NULL DEFAULT '{}',
  arrive_at       INTEGER NOT NULL,
  created_at      INTEGER NOT NULL DEFAULT (unixepoch())
);

CREATE INDEX IF NOT EXISTS idx_fleet_missions_user ON fleet_missions(user_id, arrive_at);
