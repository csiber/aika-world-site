...
module.exports = function(options = {}) {
  ...
  return fetch(`${ baseUrl }/${ archiveName }-${ platform }-${ arch }.zip`).then(archive => {
    ...
    console.warn("[rclone.js] Skipping rclone binary download due to an error.");
    if (error && error.message) {
      console.warn(error.message);
    } else {
      console.warn(error);
    }
    console.warn("[rclone.js] Install rclone manually if you rely on commands that require the binary.");
  });
}
