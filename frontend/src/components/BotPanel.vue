<template>
  <div class="bot-panel" :class="{ minimized }">
    ...
    <div class="bot-header" @click="minimized = !minimized">
      <span class="bot-title">🤖 BOT MÓD</span>
      <div class="bot-header-actions" @click.stop>
        <button class="bot-btn" :disabled="bot.status === 'exhausted'" @click="toggleBot" :title="bot.active ? 'Leállítás' : 'Indítás'">
          {{ bot.active ? '■' : '▶' }}
        </button>
        <button class="bot-btn close-btn" @click="minimized = !minimized" :title="minimized ? 'Kinyitás' : 'Minimalizálás'">
          {{ minimized ? '▲' : '▼' }}
        </button>
      </div>
    </div>
    ...
    <div v-if="!minimized" class="bot-body">
      ...
      <div class="bot-stats">
        <div class="stat-row">
          ...
          <span>Ma: <b>{{ dailyTimeStr }}</b> / {{ bot.settings.dailyTimeLimitMin }} perc</span>
        </div>
        <div class="stat-row">
          <span class="stat-icon">🎯</span>
          <span>Akciók: <b>{{ bot.todayActions }}</b> / {{ bot.settings.dailyActionLimit }}</span>
        </div>
        <div class="stat-row" v-if="bot.active">
          <span class="stat-icon">🕐</span>
          <span>Session: <b>{{ sessionStr }}</b> / {{ bot.settings.sessionTimeLimitMin }}:00</span>
        </div>
      </div>
      ...
      <div v-if="showSettings" class="bot-settings">
        ...
        <div class="notif-row">
          ...
          <button
            ...
            @click="onRequestNotif"
            :disabled="notifPermission === 'granted' || notifPermission === 'unsupported'"
          >{{ notifLabel }}</button>
        </div>
        <div v-if="notifPermission === 'granted'" class="notif-hint">
          Háttérben értesít, ha aktív a bot
        </div>
        <div v-if="notifPermission === 'denied'" class="notif-hint warn">
          Böngészőben tiltva — engedélyezd a beállításokban
        </div>
      </div>
      ...
      <div class="bot-log">
        ...
        <div v-if="bot.log.length === 0" class="log-empty">— nincs napló —</div>
      </div>
...
function onSoundToggle() {
  setVolume(soundEnabled.value ? soundVolume.value : 0);
}
...
const notifLabel = computed(() => {
  switch (notifPermission.value) {
    ...
    case 'denied':      return '✗ Tiltva';
    case 'unsupported': return 'Nem támogatott';
    default:            return 'Engedély kérése';
  }
});
  ...
  (entry) => {
    ...
    if (soundEnabled.value) {
      switch (entry.type) {
        ...
        case 'system':
          entry.msg.includes('aktiválva') ? sounds.start() : sounds.stop();
          break;
      }
    }

    // Browser notification (only if tab is hidden, not for idle)
    if (entry.type !== 'idle') {
      sendNotification('🤖 AIKA Bot', entry.msg);
    }
  }
);
...
const statusLabel = computed(() => {
  switch (bot.status) {
    ...
    case 'exhausted': return 'KIMERÜLT';
    default:          return 'MEGÁLLT';
  }
});
...
.bot-btn {
  ...
  border: 1px solid var(--border, rgba(0,200,255,0.2));
  color: var(--accent, #00c8ff);
  width: 24px;
  height: 22px;
  border-radius: 3px;
...
.status-dot {
  ...
  border-radius: 50%;
  flex-shrink: 0;
  background: var(--text-dim, #7090b0);
}
.status-dot.running {
  background: #00e676;
...
.settings-toggle {
  background: none;
  border: 1px solid var(--border, rgba(0,200,255,0.15));
